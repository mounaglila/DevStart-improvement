...
:: Récupère le chemin cible passé en dernier argument
set "ROOT_DIR=%~1"
set "host=%~2"
set "DdbName=%~3"
set "USERNAME=%~4"
...
set "jsonFile=%temp%\request.json"
echo {"host":"%host%","dbName":"%DdbName%","username":"%USERNAME%","password":"%PASSWORD%"} > "%jsonFile%"

:: Send POST request to get table names
set "apiUrl=http://localhost:5000/api/tablenames"
curl -s -X POST -H "Content-Type: application/json" -d "@%jsonFile%" %apiUrl% > "%temp%\tablenames.json"
...
for /f "delims=" %%i in ('findstr /r /c:"\".*\"" "%temp%\tablenames.json"') do (
    set name=%%i
    set name=!name:"=!
...
:: Install dependencies after generation
    ...
    move angular-project "%FRONTEND_DIR%"
    :: npm install 
    if errorlevel 1 (
    ...
    echo Angular project already exists! in 
    cd "%projectDir%"
)
