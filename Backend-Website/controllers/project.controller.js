...
const generateProject = async (req, res) => {
    ...
    try {
        ...
        const executeBackend = () => new Promise((resolve, reject) => {
            ...
            const command = `cmd /c ""${scriptPath}" "${host || ''}" "${dbName || ''}" "${username || ''}" "${password || ''}" "${port || ''}" "${projectDir}"" > log_backend_${uniqueId}.txt 2>&1`;
        ...
        const executeFrontend = () => new Promise((resolve, reject) => {
            ...
            const command = `cmd /c ""${scriptPath2}" "${projectDir}" "${dbHost}" "${databaseName}" "${dbUser}" "${dbPass}"" > log_frontend_${uniqueId}.txt 2>&1`;
            // --- End Modification --- 
